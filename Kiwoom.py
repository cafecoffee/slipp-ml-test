...
class Kiwoom(QAxWidget):
    ...
    def _create_kiwoom_instance(self):
        self.setControl("KHOPENAPI.KHOpenAPICtrl.1")  # ProgID는 윈도우 레지스트에 등록되어 있으며 키움증권 클래스를 사용하기 위해 필요함
    ...
    def comm_connect(self):
        self.dynamicCall("CommConnect()")  # 키움증권 로그인 윈도우를 실행
        self.login_event_loop = QEventLoop()
        self.login_event_loop.exec_()  # 이벤트 루프 실행 (이벤트 루프란? 무한 반복하면서 이벤트를 처리하는 상태를 의미)
